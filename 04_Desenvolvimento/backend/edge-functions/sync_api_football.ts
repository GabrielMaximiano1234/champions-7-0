...
// ============================================================================
// Function Name: sync_api_football.ts
  ...
  try {
    ...
    for (const teamId of teamIds) {
      ...
      if (coachResponse.ok) {
        ...
        if (activeCoach) {
          // Desativar técnicos anteriores deste time no banco
          await supabase.from("coaches").update({ is_active: false }).eq("team_id", teamId);
          // Atualizar ou inserir o novo técnico ativo
      ...
      if (transfersResponse.ok) {
        ...
        for (const t of transfersData.response || []) {
          for (const mov of t.transfers || []) {
            // Se o movimento de saída ocorreu em 2026 e o clube de origem é o nosso teamId
      ...
      if (squadResponse.ok) {
        ...
        for (const player of playersList) {
          // Se o jogador está na lista de saídas confirmadas (como Diogo Barbosa, Estêvão ou Veiga)
          if (transferredOutPlayerIds.has(player.id)) {
            // Marca como transferido e remove vínculo com o time
          ...
          // Caso contrário, jogador é ativo do plantel
          ...
          if (upsertedPlayer) {
            playersUpdatedCount++;
            // O trigger no banco automaticamente criará o registro em `player_career_history` em caso de mudança de time ou OVR!
          }
        }
        teamsSyncedCount++;
      }
    ...
    // Registrar auditoria de execução da sincronização
